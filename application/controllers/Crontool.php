...
class Crontool extends CI_Controller {
			...
			foreach ($socialAcc as $socRef => $fldRef) {
				if( !$urlProf[ "$fldRef_reset_token" ] ){
					...
					if( $socRef == 'analytic' ){
						...
						$opt['access_token'] = $urlProfile["$fldRef_access_token"];
						$opt['refresh_token'] = $urlProfile["$fldRef_refresh_token"];
						$client_token = $this->loaddata->updateGoogleTokens(true, $opt);
						$client = $client_token['client'];
						$this->getViewData( $profileDet, $client);
					}
					if( $socRef == 'adwords' ){
						$this->updateGoogleAdwordsData( $profileDet );
					}
				}
			}
	...
	public function checkTokenValid($profDet) {
		...
		foreach ($socialAcc as $socRef => $fldRef) {
			...
			$opt['access_token'] = $fetchedProfile["$fldRef_access_token"];
			$opt['refresh_token'] = $fetchedProfile["$fldRef_refresh_token"];
	...
	public function updateGoogleAdwordsData($profileDet) {
		...
		foreach ($months_tstamps as $mtstamp) {
			...
			$params['access_token'] = $profileDet['adword_access_token'];
			$params['refresh_token'] = $profileDet['adword_refresh_token'];
			if (!$gaData || in_array($month_ref, array($currMonthRef, $lastMonthRef))) {
